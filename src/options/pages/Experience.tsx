...
import type { Profile, InternshipEntry } from '../../shared/types';
...
const Experience: React.FC<Props> = ({ profile, onUpdate }) => {
  ...
  const update = (id: string, field: keyof InternshipEntry, value: unknown) => {
    onUpdate((prev) => ({
      ...prev,
  ...
  return (
    <div>
      <div style={styles.header}>
        <h2 style={styles.title}>💼 实习经历</h2>
        <button style={styles.addBtn} onClick={add}>+ 添加实习</button>
      </div>
      <p style={styles.hint}>填写你的实习经历。LLM 会根据表单上下文选择合适的条目来填写"实习经历"相关字段。</p>

      {internships.length === 0 && (
        <div style={styles.empty}>暂无实习经历，点击上方按钮添加</div>
      )}

      {internships.map((entry, index) => (
        <div key={entry.id} style={styles.card}>
          <div style={styles.cardHeader}>
            <span style={styles.cardTitle}>实习 #{index + 1}</span>
          ...
          <div style={styles.grid}>
            ...
            <Field label="税前薪资">
              <input style={styles.input} value={entry.salaryBeforeTax || ''} onChange={(e) => update(entry.id, 'salaryBeforeTax', e.target.value || null)} placeholder="如: 8000/月" />
            </Field>
            <Field label="离职原因">
          ...
          <div style={styles.textareaField}>
            ...
            <textarea style={styles.textarea} rows={3} value={entry.achievements || ''} onChange={(e) => update(entry.id, 'achievements', e.target.value || null)} placeholder="量化的工作成果" />
          </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  gridSmall: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '12px', marginTop: '12px' },
